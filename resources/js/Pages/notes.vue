	...
	<div class="container-fluid mt-3">
		<div class="card shadow p-3 mb-5 bg-white rounded">
		  <div class="card-body">
			        ...
			        <b-button variant="info" @click="setFormValues(row.item)" size="sm">  <b-icon icon="plus"></b-icon> Add Note</b-button>
        ...
        this.currentPage = 1
			...
			onSubmit() {
				...
				for (let i = 0; i < this.attachment.length; i++) {
					data.append('note'+[i +1], this.attachment[i]) 
				};

            	this.$inertia.post('/notes', data).then( () => {
            	...
            	/*
				axios.post('/notes',this.formData, {headers: {'Content-Type': 'multipart/form-data'}}).
				then(response => {
					            this.$noty.success(response.data.message);
					            this.attachment = null ;
					            this.show = false}).
				catch(error => this.$noty.warning(error.response.data))
				*/
			},
